...
| 规格编号 | 模块 | 代码文件 | 测试用例 |
|----------|------|----------|----------|
| CC-001 | 合同管理 | `backend/models.py`、`backend/main.py`（`/api/contracts`、`/api/contract/*`） | `tests/test_api_smoke.py::TestApiSmoke::test_contracts`、`test_stats`；`tests/test_contract_domain.py`（test_main_create_contract_is_models_version / test_post_api_contracts_creates_row / test_post_api_contracts_rejects_blank_name / test_created_contract_visible_in_list / test_delete_api_contracts_cascades / test_three_contract_domains_do_not_cross_write / test_procurement_contract_delete_still_works） |
