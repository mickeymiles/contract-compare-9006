...
- [x] [P0] T1 `backend/main.py` 新增 `no_cache_static_assets` HTTP 中间件 #CC-011
...
- [x] [P0] T1b 条件请求短路 `_etag_match()` + 304 返回 #CC-011
  - `FileResponse` 不处理 `If-None-Match`，必须在中间件内自行比对并回 304 空体
  - 按 RFC 7232 支持 `*` / 逗号列表 / `W/` 弱标记

## 测试

- [x] [P0] T2 `tests/test_static_cache.py` #CC-011
...
- [x] [P0] T3 全量回归 `pytest -q` + CI 同参数 ruff

## 收尾

- [x] [P0] T4 推送并确认 Actions run 绿、deploy 执行
- [x] [P1] T5 生产复验：`curl -D-` 看响应头；带 `If-None-Match` 复验 304
- [x] [P1] T6 归档：`archive/2026-08-27-static-asset-cache/`，delta 合并进
